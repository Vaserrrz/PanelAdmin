<div>
    <!-- BOTON AGREGAR -->
    <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#ModalAgregar">
        Crear Remota
    </button>

    <!-- MODAL AGREGAR -->
    <div class="modal fade" id="ModalAgregar" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="ModalAgregar" aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title" id="staticBackdropLabel">CREAR REMOTA</h3>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>

                <div class="modal-body">
                    <form  action="{{ route('remotas.create') }}"  method="POST" lang="es" id="Form_MA">
                        @csrf
                        {{-- NODO y Select Cliente --}}
                        <div class="row">
                            <div class="col col-md-8">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_NODO_MA">NODO</label>
                                    <input type="text" value="{{old('REMOTA_NODO_MA')}}" class="form-control" id="REMOTA_NODO_MA" wire:model='title' placeholder="Ingrese el Nodo de la Remota" name="REMOTA_NODO_MA">
                                    {{$title}}
                                </div>
                            </div>

                            <div class="col col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="SELECT_CLIENTE_MA">Cliente</label>
                                    <select id="SELECT_CLIENTE_MA" name="SELECT_CLIENTE_MA" class="form-select">
                                        <option value="">Escoga el cliente...</option>
                                        @forelse($clientes as $cliente)
                                            <option value="{{$cliente->id}}">{{$cliente->CLIENTE_RAZON}}</option>
                                        @empty
                                            <option>No hay clientes registrados</option>
                                        @endforelse
                                    </select>
                                </div>
                            </div>
                        </div>

                        {{-- Equipo y Select Encargado --}}
                        <div class="row">
                            <div class="col col-md-8">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_EQUIPO">Equipo</label>
                                    <input type="text" value="{{old('REMOTA_EQUIPO')}}" class="form-control" id="REMOTA_EQUIPO_MA" placeholder="Ingrese el Equipo de la Remota" name="REMOTA_EQUIPO_MA">
                                    @error('REMOTA_EQUIPO_MA')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>


                            <div class="col col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="SELECT_ENCARGADO_MA">Encargado</label>
                                    <select id="SELECT_ENCARGADO_MA" name="SELECT_ENCARGADO_MA" class="form-select select_encargado">
                                        <option selected>Seleccione un Encargado...</option>
                                        @forelse($encargados as $encargado)
                                            <option value="{{$encargado->id}}">{{$encargado->ENCARGADO_NOMBRE}}</option>
                                        @empty
                                        @endforelse
                                    </select>
                                </div>
                            </div>
                        </div>

                        {{-- Serial Modem/Router, Coordenadas y Socio --}}
                        <div class="row">
                            <div class="col col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="descripcion">Serial Modem/Router</label>
                                    <input type="text" value="{{old('REMOTA_SERIAL_MA')}}" class="form-control" id="REMOTA_SERIAL_MA" placeholder="Ingrese el Serial de la Remota" name="REMOTA_SERIAL_MA">
                                    @error('REMOTA_SERIAL_MA')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>

                            <div class="col col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_COORDENADA_MA">Coordenadas</label>
                                    <input type="text" value="{{old('REMOTA_COORDENADA_MA')}}" class="form-control" id="REMOTA_COORDENADA_MA" placeholder="Ingrese las Coordenadas de la Remota" name="REMOTA_COORDENADA">
                                    @error('REMOTA_COORDENADA_MA')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>

                            <div class="col col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="SELECT_SOCIO_MA">Socio</label>
                                    <select id="SELECT_SOCIO_MA" name="SELECT_SOCIO_MA" class="form-select select_socio">
                                        <option selected>Seleccione el Socio...</option>
                                        @forelse($socios as $socio)
                                            <option value="{{$socio->id}}">{{$socio->nombre}}</option>
                                        @empty
                                        @endforelse
                                    </select>
                                </div>
                            </div>
                        </div>

                        {{-- BUC y BUC Serial --}}
                        <div class="row">
                            <div class="col col-md-8">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_BUC">BUC</label>
                                    <input type="text" value="{{old('REMOTA_BUC')}}" class="form-control" id="REMOTA_BUC" placeholder="Ingrese el BUC de la Remota" name="REMOTA_BUC">
                                </div>
                            </div>


                            <div class="col col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_BUC_SERIAL">BUC Serial</label>
                                    <input type="text" value="{{old('REMOTA_BUC_SERIAL')}}" class="form-control" id="REMOTA_BUC_SERIAL" placeholder="Ingrese el Serial del BUC" name="REMOTA_BUC_SERIAL">
                                </div>
                            </div>
                        </div>

                        {{-- LNB y Serial LNB --}}
                        <div class="row">
                            <div class="col col-md-8">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_LNB">LNB</label>
                                    <input type="text" value="{{old('REMOTA_LNB')}}" class="form-control" id="REMOTA_LNB" placeholder="Ingrese el LNB de la Remota" name="REMOTA_LNB">
                                    @error('REMOTA_LNB')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>


                            <div class="col col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_LNB_SERIAL">Serial LNB</label>
                                    <input type="text" value="{{old('REMOTA_LNB_SERIAL')}}" class="form-control" id="REMOTA_LNB_SERIAL" placeholder="Ingrese el Serial del LNB" name="REMOTA_LNB_SERIAL">
                                    @error('REMOTA_LNB_SERIAL')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>
                        </div>

                        {{-- Serial Antena y Serial Kit --}}
                        <div class="row">
                            <div class="col col-md-6">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_ANTENA_SERIAL">Serial Antena </label>
                                    <input type="text" value="{{old('REMOTA_ANTENA_SERIAL')}}" class="form-control" id="REMOTA_ANTENA_SERIAL" placeholder="Ingrese el Serial de la Remota" name="REMOTA_ANTENA_SERIAL">
                                </div>
                            </div>

                            <div class="col col-md-6">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_KIT_SERIAL">Serial Kit</label>
                                    <input type="text" value="{{old('REMOTA_KIT_SERIAL')}}" class="form-control" id="REMOTA_KIT_SERIAL" placeholder="Ingrese el Serial de la Remota" name="REMOTA_KIT_SERIAL">
                                </div>
                            </div>
                        </div>

                        {{-- Dia Activacion, Dia corte y Fecha cuenta --}}
                        <div class="row">
                            <div class="col col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_DIA_ACTIVACION_MA">Dia Activacion</label>
                                    <input type="text" value="{{old('REMOTA_DIA_ACTIVACION_MA')}}" class="form-control" id="REMOTA_DIA_ACTIVACION_MA" placeholder="Ingrese el Dia de Activacion de la Remota" name="REMOTA_DIA_ACTIVACION_MA" data-flatpickr>
                                </div>
                            </div>
                            <div class="col col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_DIA_CORTE_MA">Dia de Corte</label>
                                    <input type="text" value="{{old('REMOTA_DIA_CORTE_MA')}}" class="form-control" id="REMOTA_DIA_CORTE_MA" placeholder="Ingrese el dia de corte de la Remota" name="REMOTA_DIA_CORTE_MA" data-flatpickr>
                                </div>
                            </div>
                            <div class="col col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_FECHA_CUENTA">Fecha Cuenta</label>
                                    <input type="date" value="{{old('REMOTA_FECHA_CUENTA')}}" class="form-control" id="REMOTA_FECHA_CUENTA" placeholder="seleccione una fecha" name="REMOTA_FECHA_CUENTA" data-flatpickr>
                                </div>
                            </div>
                        </div>

                        {{-- Detalle y Status --}}
                        <div class="row">
                            <div class="col col-md-8">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_DETALLE">Detalle</label>
                                    <input type="text" value="{{old('REMOTA_DETALLE')}}" class="form-control" id="REMOTA_DETALLE" placeholder="Detalles a agregar" name="REMOTA_DETALLE">
                                </div>
                            </div>

                            <div class="col col-md-4">
                                <div class="form-check mb-5">
                                    <label class="form-label" for="Checkbox Status">Status</label>
                                    <input type="checkbox" value="{{old('REMOTA_STATUS')}}" class="form-check-input" id="REMOTA_STATUS" placeholder="Ingrese el Status de la Remota" name="REMOTA_STATUS">
                                </div>
                            </div>
                        </div>

                        {{-- IP Modem y IP Gestion,  --}}
                        <div class="row">
                            <div class="col col-md-6">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_IP_MODEM">IP Modem</label>
                                    <input type="text" value="{{old('REMOTA_IP_MODEM')}}" class="form-control" id="REMOTA_IP_MODEM" placeholder="Ingrese el IP del Modem" name="REMOTA_IP_MODEM">
                                    @error('REMOTA_IP_MODEM')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>

                            <div class="col col-md-6">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_IP_GESTION">IP Gestion</label>
                                    <input type="text" value="{{old('REMOTA_IP_GESTION')}}" class="form-control" id="REMOTA_IP_GESTION" placeholder="Ingrese el IP de Gestion" name="REMOTA_IP_GESTION">
                                    @error('REMOTA_IP_GESTION')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>
                        </div>

                        {{-- Plato y Bonda,  --}}
                        <div class="row">
                            <div class="col col-md-6">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_PLATO">Plato</label>
                                    <input type="text" value="{{old('REMOTA_PLATO')}}" class="form-control" id="REMOTA_PLATO" placeholder="Ingrese el Plato de la Remota" name="REMOTA_PLATO">
                                    @error('REMOTA_PLATO')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>
                            <div class="col col-md-6">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_BONDA">Bonda</label>
                                    <input type="text" value="{{old('REMOTA_BONDA')}}" class="form-control" id="REMOTA_BONDA" placeholder="Ingrese la Bonda de la Remota" name="REMOTA_BONDA">
                                    @error('REMOTA_BONDA')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>
                        </div>

                        {{-- SELECTS: Proveedores, Satelites, Planes y Revendedores --}}
                        <div class="row">
                            {{-- PROVEEDORES --}}
                            <div class="col col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="SELECT_PROVEEDOR_MA">Proveedores</label>
                                    <select id="SELECT_PROVEEDOR_MA" name="SELECT_PROVEEDOR_MA" class="form-select">
                                        <option value="" value="">Seleccione un Proveedor...</option>
                                        @forelse($proveedores as $proveedor)
                                            <option value="{{$proveedor->id}}">{{$proveedor->razon}}</option>
                                        @empty
                                            No hay Satelites registrados
                                        @endforelse
                                    </select>
                                    @error('SELECT_PROVEEDOR_MA')
                                        <br>
                                        <small>
                                            *{{$message}}
                                        </small>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>

                            {{-- SATELITES --}}
                            <div class="col col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="SELECT_SAT_MA">Satelites</label>
                                    <select id="SELECT_SAT_MA" name="SELECT_SAT_MA" class="form-select" >
                                        <option value="">Seleccione el Satelite...</option>
                                    </select>
                                    @error('SELECT_SAT_MA')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>

                            {{-- PLAN --}}
                            <div class="col col-md-4">
                                <div class="mb-3">
                                    <label class="form-label" for="SELECT_PLAN_MA">Plan</label>
                                    <select class="form-select" id="SELECT_PLAN_MA" name="SELECT_PLAN_MA">
                                        <option value="">Seleccione un plan . . . </option>
                                    </select>
                                </div>
                            </div>

                            {{-- REVENDEDOR --}}
                            <div class="col col-md-12">
                                <div class="mb-3">
                                    <label class="form-label" for="inputState">Revendedor</label>
                                    <select id="SELECT_RESELLER" name="SELECT_RESELLER" class="form-select">
                                        <option selected>Escoga el Revendedor...</option>
                                        @forelse($revendedores as $revendedor)
                                            <option value="{{$revendedor->id}}">{{$revendedor->NOMBRE_RESELLER}}</option>
                                        @empty
                                        @endforelse
                                    </select>
                                </div>
                            </div>
                        </div>

                        {{-- Propiedades de Plan --}}
                        <div class="row">
                            <div class="col col-md-3">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_PLANUP">PlanUp</label>
                                    <input type="text" value="{{old('REMOTA_PLANUP')}}" class="form-control" id="REMOTA_PLANUP" placeholder="Ingrese el PLANUP de la Remota" name="REMOTA_PLANUP">
                                    @error('REMOTA_PLANUP')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>
                            <div class="col col-md-3">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_PLANDOWN">PlanDown</label>
                                    <input type="text" value="{{old('REMOTA_PLANDOWN')}}" class="form-control" id="REMOTA_PLANDOWN" placeholder="Ingrese el PLANDOWN de la Remota" name="REMOTA_PLANDOWN">
                                    @error('REMOTA_PLANDOWN')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>

                            <div class="col col-md-3">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_COSTO_PLAN">Costo del Plan</label>
                                    <input type="text" value="{{old('REMOTA_COSTO_PLAN')}}" class="form-control" id="REMOTA_COSTO_PLAN" placeholder="Ingrese el Costo del Plan" name="REMOTA_COSTO_PLAN">
                                    @error('REMOTA_COSTO_PLAN')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>

                            <div class="col col-md-3">
                                <div class="mb-3">
                                    <label class="form-label" for="REMOTA_RENTA">Renta</label>
                                    <input type="text" value="{{old('REMOTA_RENTA')}}" class="form-control" id="REMOTA_RENTA" placeholder="Ingrese el monto de la Renta" name="REMOTA_RENTA">
                                    @error('REMOTA_RENTA')
                                        <br>
                                            <small>
                                                *{{$message}}
                                            </small>
                                        <br>
                                    @enderror
                                </div>
                            </div>
                        </div>
                    </form>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                    <button type="button" class="btn btn-primary">Guardar</button>
                </div>
            </div>
        </div>
    </div>
</div>
